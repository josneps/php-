...
class AskDesignerController extends BaseController
{
    ...
    public function index()
    {
        ...
        if($data){
            ...
            //未采纳的解答信息
            $no_adopted_data = array();
            if(!empty($no_adopted_ids)){
                $no_adopted_data = M('designer_answer')->where('a_questions_id in ('.$no_adopted_ids.')')->select();
            }

            $mid = array();
            if(!empty($adopted_data) || !empty($no_adopted_data)){
...
//                    $answerAdopt = M('answer_adopt_num')->field('a_mid mid, answer, adopt')->where("a_mid in (".$mids.")")->select();
        ...
        //判断当前登录的是业主&设计师
        if(!empty($this->userInfo) && $this->userInfo['type'] == 1){
            $type = 1;
        } elseif (!empty($this->userInfo) && $this->userInfo['type'] == 0) {
            $type = 2;
        } else {
            $type = 3;
        }
    ...
    public function details()
    {
        ...
        if($detail_data !== null){
            ...
            foreach ($img as $k => $val) {
                if(strpos($val, '@dbw800')  !== false) {
                    $val = str_replace('@dbw800', '', $val);
                }

                $img[$k] = C('PIC_URL').'/'.$val.'@dbw800';
            }
            ...
            //获取设计师信息
            $designer_data = $this->randomGetDesigner($mids);
            ...
            if($answer_one) {
                ...
                if($designer_info[$answer_one['a_mid']]['nickname_type'] == 1){
                    $answer_one['a_nickname'] = $designer_info[$answer_one['a_mid']]['nickname'];
                } else {
            ...
            if(IS_POST) {
                ...
                foreach ($data as $key => $val) {
                    ...
                    //组合设计师的昵称
                    if($designer_info[$data[$key]['a_mid']]['nickname_type'] == 1){
                        $data[$key]['a_nickname'] = $designer_info[$data[$key]['a_mid']]['nickname'];
                    } else {
            ...
            //当访问一个不存在的时候进来
            $this->error('数据不存在！',U('Askdesigner/index'),3);
        }
     ...
     * @return mixed
     */
    public function randomGetDesigner($mids, $num = 6)
    {
